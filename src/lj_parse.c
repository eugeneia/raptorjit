...
static LJ_AINLINE VarHash var_hash(GCstr *name)
{
  if ((uintptr_t)name < VARNAME__MAX)
    ...
    return (name->hash & LJ_VINDEX_MASK);  /* Immutable id, not name->hash! */
}
...
static GCproto *fs_finish(LexState *ls, BCLine line, const char *declname)
{
  lua_State *L = ls->L;
  FuncState *fs = ls->fs;
                         ...
                         const char *declname)
{
  ...
  GCproto *pt = fs_finish(ls, (ls->lastline = ls->linenumber), declname);
  FuncState *pfs = ls->fs;
                       ...
                       const char *declname)
{
  ...
  proto_finish(ls, e, oldbase, declname);
  lj_lex_next(ls);
}
			    ...
			    int eflags, BCLine line)
{
  ...
  proto_finish(ls, e, oldbase, strdata(name));
}
